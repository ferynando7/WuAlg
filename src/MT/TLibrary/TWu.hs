...
getPseudoRemainders pols var = parMap rpar (pseudoRemainder var divisor) dividens
        where
...
invPseudoRemainders pols pol = parMap rpar (foo pol) polinomials
        where
...
characteristicWuSet polys [] var = (basisPoly: characteristicWuSet pseudos [basisPoly] (var+1))
        ...
        pseudos = parMap rpar (\p -> if p == basisPoly && minimalPoly /=  basisPoly then pseudoRemainder var basisPoly minimalPoly else pseudoRemainder var basisPoly p) (getPseudoRemainders polys var)
characteristicWuSet polys oldChain var =  (basisPoly : characteristicWuSet pseudos (oldChain ++ [basisPoly]) (var+1) )
        ...
        -- We compute the pseudo remainders for the next iteration
        pseudos = parMap rpar (\p -> if p == basisPoly && basisPoly /= minimalPoly then pseudoRemainder var basisPoly minimalPoly else pseudoRemainder var basisPoly p) (getPseudoRemainders polys var)
