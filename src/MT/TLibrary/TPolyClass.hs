...
import MT.TLibrary.TPolyCompare
...
leadingTerm' pol var = (snd &&& fst) $ fromJust $ M.lookupLE chosenTerm (_terms pol)
        where
                chosenTerm = toMonomial (foldr' foo (last polToList) (init polToList))
                polToList = parMap rpar (V.toList . getMonomial) (M.keys $ _terms pol)
...
chooseTermsWithVar pol var
                ...
                where
                        ...
                        idxs = findIndices (\x -> x!!var == deg_pol) (parMap rpar (V.toList . getMonomial) (M.keys $ _terms pol))
...
pol // (coeff, mon) = sum $ parMap rpar toPolynomial $ parMap rpar (`tryDiv'` (coeff, mon)) (parMap rpar (snd &&& fst) terms)
            where
