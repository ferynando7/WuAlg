...
import MT.TLibrary.TMon
import MT.TLibrary.TPolyClass
...
simplifyNumSym pol =  Polynomial $ M.fromList $ parMap rpar (\(mon, coef)  -> (mon,  toExpr $ M.map (`div` gcdnum) $ fromExpr coef) ) $ M.toList  $ _terms pol
                where
                  coefficients =  parMap rpar M.elems $ parMap rpar (fromExpr) $  parMap rpar (snd) $  M.toList $ _terms pol
                  -- Here we obtain the coefficients of the polynomial
                  gcdnum =  foldl1' gcd $ parMap rpar (foldl1' gcd) $ coefficients
...
getPseudoRemaindersSym pols var = parMap rpar (pseudoRemainderSym var divisor) dividends
        where
...
invPseudoRemaindersSym pols pol = parMap rpar (foo pol) polinomials
        where
...
characteristicWuSetSym polys [] var = (basisPoly: characteristicWuSetSym pseudos [basisPoly] (var+1))
        ...
        pseudos = parMap rpar (\p -> if p == basisPoly && minimalPoly /=  basisPoly then pseudoRemainderSym var basisPoly minimalPoly else pseudoRemainderSym var basisPoly p) (getPseudoRemaindersSym polys var)
characteristicWuSetSym polys oldChain var =  (basisPoly : characteristicWuSetSym pseudos (oldChain ++ [basisPoly]) (var+1) )
        ...
        -- We compute the pseudo remainders for the next iteration
        pseudos = parMap rpar (\p -> if p == basisPoly && basisPoly /= minimalPoly then pseudoRemainderSym var basisPoly minimalPoly else pseudoRemainderSym var basisPoly p) (getPseudoRemaindersSym polys var)
...
changeVariables pol step coeff = Polynomial $ M.fromList $ zipWith (\(a,b) c -> (a,c)) (reverse $ M.toList $ terms  pol) (Algebra.Prelude.map ( fromCoeffT step) [coeff ..])
...
changeVariablesList (x:xs) var coeff = (newPolX : changeVariablesList xs var (succ lastCoeff))
        where
                ...
                lastCoeff = (toCoeffT . snd . head . M.toList . terms) newPolX
...
newAscChain pols var = do
                ...
                let thingToSave = ( \(a,b) -> (parMap rpar simplifyNumSym a, parMap rpar simplifyNumSym b) ) $ characteristicWuSingletonSym pols [] var
...
printCoeffs new@(n:ns) old@(o:os) path = do
                                        let coeffsNew = ((parMap rpar (snd)) . reverse . M.toList . terms) n
                                        let coeffsOld = ((parMap rpar (snd)) . reverse . M.toList . terms) o
...
--        |otherwise = Polynomial $ M.fromList $ zipWith (,) newAlgPart coeffs
                where
                        ...
                        algPart = parMap rpar fst polToList
                        coeffs = parMap rpar snd polToList
                        newAlgPart = parMap rpar (toMonomial) $ (parMap rpar (tail . S.toList . getMonomial)) algPart
...
evaluatePoly poly (str, val) =  Polynomial $ M.fromList  $ evaluateCoef polyList
        where
                ...
                evaluateCoef = parMap rpar (\(mon, coeff) ->  (mon, evaluate coeff str val))
